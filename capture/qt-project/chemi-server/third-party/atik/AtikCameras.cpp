...
typedef int(__stdcall *AtikMethod_I_HBQ)(	 ArtemisHandle, bool, ARTEMISPREVIEWQUALITY);
...
extern "C"
{
	...
	{
		static AtikMethod_I_HBQ method = NULL;

		if (method == NULL)
			method = (AtikMethod_I_HBQ)GetProcAddress(hArtemisDLL, "ArtemisSetPreviewEx");

		if (method == NULL)
			return FALSE;

		return method(handle, bPrev, quality);
	}
	...
	int ArtemisAutoHotPixelRemoval(ArtemisHandle handle, bool enable)
	{
		static AtikMethod_I_HB method = NULL;
	...
	int ArtemisAdvancedHotPixelRemoval(ArtemisHandle handle, bool enable, bool darkFrame, bool checkForAdjacentHotPixels, HotPixelSensitivity hps)
	{
    ...
    bool artfn ArtemisLoadDLL(const wchar_t *fileName)
	{
        std::wstring fileStr = fileName;

#if _WIN32
        hArtemisDLL = LoadLibraryEx(fileStr.c_str(), NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
#else
