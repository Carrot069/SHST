...
bool WzKsjCamera::openCamera() {
    ...
    retCode = KSJ_CaptureGetSizeExEx(m_deviceCurSel, &m_SensorResX, &m_SensorResY,
                                     &m_imageBitCount, &nBitsPerSample);
    ...
    if (m_deviceInfo[m_deviceCurSel].DeviceType == KSJ_UD205M_SGYY) {
        ...
        retCode = KSJ_SetParam(m_deviceCurSel, KSJ_GAMMA, -4);
...
//        binning = 4;
    ...
    if (getParam("grayAccumulate", isGrayAccumulate) && isGrayAccumulate == true) {
        ...
        memset(m_grayAccumulateBuffer, 0, m_imageSize->width() * m_imageSize->height());
    }
...
//        binning = 4;
    ...
    imageBytes = static_cast<uint32>(imageWidth * imageHeight * (m_bitsPerSample >> 3));
    ...
    if (checkRet(retCode, "KSJ_SetParam"))
        return false;

    for (int i = 0; i < 2; i++) {
        QElapsedTimer et;
        et.start();
        retCode = KSJ_CaptureRawDataAfterEmptyFrameBuffer(m_deviceCurSel, reinterpret_cast<unsigned char*>(m_singleFrame));
