...
void WzImageView::fit(int destWidth, int destHeight) {
    ...
    // 按照空隙少的那一边进行对齐(到容器边缘)
    int horzSpacing = destWidth - m_image->width();
    int vertSpacing = destHeight - m_image->height();
    double scale = 0;
    if (horzSpacing < vertSpacing) {
        scale = static_cast<double>(horzSpacing) / static_cast<double>(m_image->width());
    } else {
        scale = static_cast<double>(vertSpacing) / static_cast<double>(m_image->height());
    }
    setZoom(scale * 100 + 100 - 0.5);
}
