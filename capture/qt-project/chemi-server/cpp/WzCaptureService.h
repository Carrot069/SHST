#ifndef WZCAPTURE_SERVICE_H
#define WZCAPTURE_SERVICE_H

#ifdef ATIK
#include <comdef.h>
#endif

#include <QObject>
#include <QVariant>
#include <QDebug>
#include <QJsonObject>
#include <QWebSocketServer>
#include <QWebSocket>
#include <QVariantMap>
#include <QProcessEnvironment>

#include "WzDatabaseService.h"
#include "WzUtils.h"
#include "WzLiveImageView.h"
#include "WzGlobalEnum.h"
#include "WzCamera.h"
#ifdef PVCAM
#include "WzPvCamera.h"
#endif
#ifdef ATIK
#include "WzAtikCamera.h"
#endif
#ifdef KSJ_CAMERA
#include "WzKsjCamera.h"
#endif
#ifdef HARDLOCK
#include "RY3_API.h"
#include "WzSetting2.h"
#endif
#include "WzTcpServer.h"
#include "WzFileServer.h"
#include "WzDiskUtils.h"
#include "WzAutoFocus.h"
#include "WzUdpBroadcastSender.h"
#include "cpp/ShstFileServer.h"

class WzCaptureService : public QObject, public WzCameraCallback
{
    Q_OBJECT
public:

    enum CaptureState {None,
                       Init,     // 正在进行初始化，开始曝光之前的所有动作都列入该状态
                       AutoExposure, // 正在自动曝光
                       AutoExposureFinished, // 自动曝光完成
                       Exposure, // 正在曝光
                       Image,    // 曝光结束后所有动作都列入该状态
                       Finished, // 处理完图片且保存到硬盘, 所有动作都完成后进入该状态
                       Aborted   // 已取消
                      };
    Q_ENUM(CaptureState)

    explicit WzCaptureService(QObject *parent = nullptr);
    ~WzCaptureService();

    Q_INVOKABLE void initCameras();
    Q_INVOKABLE void connectCamera();
    Q_INVOKABLE void disconnectCamera();
    Q_INVOKABLE bool enableCameraPreview(bool enabled, int binning);
    Q_INVOKABLE bool resetPreview(); // 修改相机参数后重启预览, 比如binning和曝光时间
    Q_INVOKABLE bool capture(QVariantMap params);
    Q_INVOKABLE bool captureMulti(QVariantMap params);
    Q_INVOKABLE void abortCapture();
    Q_INVOKABLE int getCameraTemperature();
    Q_INVOKABLE void setCameraParam(const QString& paramName, const QVariant paramVal);
    Q_INVOKABLE int getMarkerImage(const int avgGrayThreshold);
    Q_INVOKABLE void startAutoFocus();
    Q_INVOKABLE void stopAutoFocus();

    Q_PROPERTY(int previewExposureMs MEMBER m_previewExposureMs WRITE setPreviewExposureMs) // 预览的曝光时间
    Q_PROPERTY(int exposurePercent MEMBER m_exposurePercent)              // 当前曝光时间百分比
    Q_PROPERTY(int exposureMs MEMBER m_exposureMs)                        // 当前曝光毫秒数
    Q_PROPERTY(int captureCount MEMBER m_captureCount)                    // 拍摄次数
    Q_PROPERTY(int captureIndex MEMBER m_captureIndex)                    // 当前拍摄的次数-1
    Q_PROPERTY(int capturedCount MEMBER m_capturedCount)                  // 已经拍完的数量
    Q_PROPERTY(WzCameraState::CameraState cameraState READ cameraState NOTIFY cameraStateChanged)
    Q_PROPERTY(QString leftExposureTime MEMBER m_leftExposureTime)        // 本次拍摄剩余曝光时间
    Q_PROPERTY(int leftExposureMs MEMBER m_leftExposureMs)                // 本次拍摄剩余曝光毫秒数
    Q_PROPERTY(QString elapsedExposureTime MEMBER m_elapsedExposureTime)  // 本次拍摄已经曝光的时间
    Q_PROPERTY(int elapsedExposureMs MEMBER m_elapsedExposureMs)          // 本次拍摄已经曝光的毫秒数
    Q_PROPERTY(QString latestImageFile MEMBER m_latestImageFile)          // 最新完成拍摄的图片文件名
    Q_PROPERTY(QString latestThumbFile MEMBER m_latestThumbFile)          // 最新完成拍摄的缩略图文件名
    Q_PROPERTY(int cameraCount READ getCameraCount NOTIFY cameraCountChanged)
    Q_PROPERTY(QString imagePath READ imagePath WRITE setImagePath)
    Q_PROPERTY(bool isAutoExposure READ getAutoExposure WRITE setAutoExposure)
    Q_PROPERTY(int autoExposureMs READ getAutoExposureMs)
    Q_PROPERTY(bool isGel READ getIsGel)

    Q_PROPERTY(int listenPort READ getListenPort WRITE setListenPort)

    bool cameraSN(const QString sn) override;

    int getListenPort() const;
    void setListenPort(int listenPort);

signals:
    void cameraCountChanged(const int count);
    void cameraStateChanged(const WzCameraState::CameraState& state);
    void captureState(const CaptureState& state);
    void previewImageRefreshed();  
    void tcpClientDisconnected(const int connectionCount);

    void autoFocusFar(const int step);
    void autoFocusNear(const int step);
    void autoFocusStop();
    void autoFocusFinished();
    void autoFocusNewFrame();

private slots:
    void cameraCountSlot(WzAbstractCamera *sender, const int count);
    void previewImageUpdated();
    void captureTimer();
    void cameraStateUpdated(const WzCameraState::CameraState& state);

    // auto focus
    void autoFocusGetImage();
    void autoFocusLog(const QString &msg);
    void autoFocusStopSlot();
    void autoFocusFinishedSlot();

    void onWebSocketNewConnection();
    void onWebSocketDisconnected();
    void textMessageReceived(const QString &message);

private:
    QVariantMap m_cameraParams;
    WzAbstractCamera* m_camera = nullptr; // 当前相机
    QList<WzAbstractCamera*> m_cameras; // 所有种类的相机的实例
    int m_searchedCameraTypeCount = 0; // 已查找过的相机种类的数量
    int m_cameraCount = 0; // 所有种类的相机数量
    uchar* m_previewImageData;
    QMutex m_previewImageDataLock;
    QSize* m_previewImageSize;
    uint64_t m_previewCount = 0;
    QTimer* m_captureTimer;
    QVariantMap m_captureParams;
    bool m_isPreviewUpdated = false;
    bool m_isCaptureFinished = false;

    // 如果正在预览，点击拍摄后此数据中保存的是预览的最后一张图片，保存下来用于叠加显示暗场图
    uint16_t* m_markerTiff16bit = nullptr;
    QSize m_markerImageSize;

    int m_previewExposureMs;             // 预览的曝光时间
    int m_exposurePercent;               // 当前曝光时间百分比
    int m_exposureMs = 0;                // 当前曝光毫秒数
    int m_captureCount;                  // 拍摄次数
    int m_captureIndex;                  // 当前拍摄的次数-1
    int m_capturedCount;                 // 已经拍完的数量
    int64_t m_captureId = 0;             // 本次拍摄的ID，多帧拍摄时每一帧的ID相同，这个ID取自1970年以来的秒数
    QString m_leftExposureTime;          // 本次拍摄剩余曝光时间
    int m_leftExposureMs = 0;            // 本次拍摄剩余曝光毫秒数
    QString m_elapsedExposureTime;       // 本次拍摄已经曝光的时间
    int m_elapsedExposureMs = 0;         // 本次拍摄已经曝光毫秒数
    QString m_latestImageFile;
    QString m_latestThumbFile;
    bool m_cameraConnected = false;
    QString m_imagePath = "";
    bool m_isAutoExposure = false;
    bool m_isGel = false;

    WzCameraState::CameraState m_cameraState;

    QList<QString> m_cameraSNList;

    QVariantMap m_imageInfo;
    QList<QVariantMap> m_cameraStateList;
    QWebSocketServer *m_pWebSocketServer = nullptr;
    QList<QWebSocket*> m_clients;
    int m_webSocketServerPort = 60052;
    WzTcpServer* m_previewServer = nullptr;
    int m_previewServerPort = 60051;
    int m_fileServerPort = 60054;    
    WzFileServer *m_fileServer = nullptr;
    ShstFileServer *m_fileServer2 = nullptr;
    int m_udpBroadcastPort = 30030;
    WzUdpBroadcastSender *m_udpBroadcastSender = nullptr;
    QImage* m_previewImage = nullptr;
    QVector<QRgb> m_colorTable;

    int getCameraCount();
    void setPreviewExposureMs(int exposureMs);
    QString saveMarkerImage(const QDateTime &dateTime);
    void readCameraSNList(); // 读取允许的使用的相机序列号列表文件
    QString imagePath();
    void setImagePath(const QString imagePath);
    WzCameraState::CameraState cameraState();
    bool getAutoExposure();
    void setAutoExposure(bool autoExposure);
    int getAutoExposureMs();

    void updateImage(const uchar* imageData, const QSize& imageSize);
    //QJsonObject captureByNetCamera(const QJsonDocument &json);
    void sendJson(const QJsonObject &json);
    void sendFileToClient(const QString &fileName);

    CaptureState m_captureState;
    void newCaptureState(CaptureState state);
    void newCameraState(const WzCameraState::CameraState &state);
    void updateCameraParam();

    void autoClearImages(WzDatabaseService &dbService);

    WzAutoFocus m_autoFocus;
    QRect m_autoFocusRect;
    bool m_isAutoFocusing = false;

    bool getIsGel() const;

    void readPorts();
};

#endif // WZCAPTURE_SERVICE_H
