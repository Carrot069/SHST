    ...
    m_imageSize(new QSize(DEMO_CAMERA_WIDTH, DEMO_CAMERA_HEIGHT))
{
    ...
    m_imageBytes = DEMO_CAMERA_WIDTH * DEMO_CAMERA_HEIGHT * 2;
    m_imageData = new uint16_t[m_imageBytes];
...
WzTestCamera::~WzTestCamera()
{
    ...
    if (nullptr != m_fakePreviewImage) {
        delete [] m_fakePreviewImage;
        m_fakePreviewImage = nullptr;
    }
    ...
    while (m_run) {
        if (m_isPreviewEnabled) {
            if (!isStartedPreview) {
                ...
                m_previewExposureMs = exposureMs.toInt();
                m_cameraState = WzCameraState::PreviewStarted;
                emit cameraState(m_cameraState);
            } else {
                m_mutexImageData.lock();

                QThread::msleep(m_previewExposureMs);
                memcpy(m_imageData, &m_fakePreviewImage[previewImageIndex * m_imageSize->width() * m_imageSize->height()],
                        m_imageBytes * 2);
                previewImageIndex++;
                if (previewImageIndex == 9)
                    previewImageIndex = 0;
                uint16_t* pSrc = m_imageData;
                uchar* pDest = m_previewImageData;
                for(int j=0; j<m_imageBytes; j++) {
        ...
        if (m_isCapture) {
            ...
            if (m_leftExposureMilliseconds <= 0) {
                ...
                m_imageBytes = m_imageSize->width() * m_imageSize->height() * 2;
                rg.fillRange(reinterpret_cast<uint*>(m_imageData), m_imageBytes/2);
                processImage();
                m_mutexImageData.unlock();
...
int WzAbstractCamera::getMarkerImage(uint16_t **)
{
    return 0;
}
...
int WzTestCamera::setPreviewEnabled(bool enabled)
{
    if (enabled) {
        ...
        m_fakePreviewImage = new uint16_t[m_imageSize->width() * m_imageSize->height() * 10];
        rg.fillRange(reinterpret_cast<uint*>(m_fakePreviewImage),
                     m_imageSize->width() * m_imageSize->height() * 5);
...
void WzTestCamera::resetPreview()
{
    ...
    getParam("ExposureMs", exposureMs);
    m_previewExposureMs = exposureMs.toInt();
    qDebug() << "TestCamera::resetPreview()";
}
...
int WzTestCamera::getMarkerImage(uint16_t **buffer)
{
    if (*buffer != nullptr) {
        delete [] (*buffer);
        *buffer = nullptr;
    }
    *buffer = new uint16_t[m_imageSize->width() * m_imageSize->height()];
    int countOfBytes = m_imageSize->width() * m_imageSize->height() * sizeof(uint16_t);
    QRandomGenerator rg;
    rg.fillRange(reinterpret_cast<uint*>(*buffer), m_imageSize->width() * m_imageSize->height() / 2);
    return countOfBytes;
}
