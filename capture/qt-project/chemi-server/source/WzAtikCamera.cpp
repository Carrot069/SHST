...
bool WzAtikCamera::openCamera() {
    ...
    if (m_numberOfCameras == 0) {
        ...
        qInfo() << "Camera Count:" << m_numberOfCameras;
    }

    int iDevice = 0;
    while (iDevice < m_numberOfCameras) {
        handle = ArtemisConnect(iDevice);
        if (handle != NULL) {
            break;
        }
        iDevice++;
    ...
    if (!ArtemisDeviceSerial(iDevice, m_camSerNum)) {
        return false;
    }
...
bool WzAtikCamera::getLatestFrame(uint16_t *frameAddress, const int timeout) {
    ...
    if (imageBuffer == NULL)
    {
        qWarning("Get Image Buffer", error);
        return false;
    }
