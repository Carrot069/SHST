...
void LiveImageView::paint(QPainter *painter)
{
    ...
    m_imageRect = WzUtils::fitRect(painterRect, imgRect, m_imageScale, true, true);

    if (m_image) {
        if (flipHoriz()) {
            QImage img = m_image->scaled(m_imageRect.width(), m_imageRect.height(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation).mirrored(true, false);
            painter->drawImage(m_imageRect, img);
        } else {
            QImage img = m_image->scaled(m_imageRect.width(), m_imageRect.height(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
            painter->drawImage(m_imageRect, img);
        }
    }
}

void LiveImageView::updateImage(const uchar *imageData, const QSize &imageSize, const int imageBytes, const QImage::Format format)
{
    if (imageSize.width() != m_image->width() ||
            imageSize.height() != m_image->height()) {
        delete m_image;
        m_image = new QImage(imageSize, format);
    }

    void* pImageData = m_image->bits();
    memcpy(pImageData, imageData, static_cast<size_t>(imageBytes));
    this->update();
}
...
void LiveImageView::updateImage(const uchar* imageData, const QSize& imageSize)
{
    ...
    void* pImageData = m_image->bits();
    memcpy(pImageData, imageData,
           static_cast<size_t>(imageSize.width()*imageSize.height()));
...
bool LiveImageView::flipHoriz() const
{
    return m_flipHoriz;
}

void LiveImageView::setFlipHoriz(bool newFlipHoriz)
{
    m_flipHoriz = newFlipHoriz;
}
