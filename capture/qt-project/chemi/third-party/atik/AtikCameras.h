...
#include <stdbool.h>
	...
	enum ARTEMISPREVIEWQUALITY 
	{
		ARTEMIS_PREVIEW_QUALITY_FULL_QUALITY,
		ARTEMIS_PREVIEW_QUALITY_QUALITY, 
		ARTEMIS_PREVIEW_QUALITY_AVERAGE,
		ARTEMIS_PREVIEW_QUALITY_SPEED 
	};
	...
	/// @param handle the connected Atik device handle.
	/// @param bPrev whether to enable or disable preview mode.
	/// @param quality Sets a tradeoff between speed and quality.
	/// @see ARTEMISERROR
	/// @return ARTEMIS_OK on success, or ARTEMISERROR enumeration on failure
	artfn int  ArtemisSetPreviewEx(ArtemisHandle handle, bool bPrev, enum ARTEMISPREVIEWQUALITY quality);
    ...
    artfn bool ArtemisLoadDLL(const wchar_t * fileName);
