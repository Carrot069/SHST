...
int64_t WzAutoFocus::getDiffCount(uint8_t *Bitmap, QSize size, QRect rect)
{
    ...
    for(i = 0; i < count; i++)
        standardDeviation += pow(static_cast<float>(Bitmap[i]) - mean, 2);
    ...
    for (int LRow = rect.top(); LRow <= rect.bottom() - 2; LRow++) {
        ...
        for (int LCol = rect.left(); LCol <= rect.right() - 2; LCol++) {
            if (bmp[RGBPtr[1]] > bmp[RGBPtr[0]]) {
                int diff = bmp[RGBPtr[1]] - bmp[RGBPtr[0]];
                // > 10%
                if (diff * 10 >= bmp[RGBPtr[1]])
                    LDiffCount += diff;
            } else {
                int diff = bmp[RGBPtr[0]] - bmp[RGBPtr[1]];
                // > 10%
                if (diff * 10 >= bmp[RGBPtr[0]])
                    LDiffCount += diff;
            }
            if (bmp[RGBPtr[1]] > bmp[RGBPtr[2]]) {
                int diff = bmp[RGBPtr[1]] - bmp[RGBPtr[2]];
                // > 10%
                if (diff * 10 >= bmp[RGBPtr[1]])
                    LDiffCount += diff;
            } else {
                int diff = bmp[RGBPtr[2]] - bmp[RGBPtr[1]];
                // > 10%
                if (diff * 10 >= bmp[RGBPtr[2]])
                    LDiffCount += diff;
            }
#if 0
...
int64_t WzAutoFocus::getDiffCount(uint16_t *Bitmap, QSize size, QRect rect)
{
#if 1
    float sum = 0;
    float mean, standardDeviation = 0;
...
bool WzAutoFocus::checkLine()
{
    ...
    // 出现下降趋势
        ...
        // 有过上升之后的下降才算聚焦清楚了
            ...
            tmrTurnBack.setInterval(trunc((COUNT_OF_LINE + 1) * (TIME_S1 + TIME_S2)));
            tmrTurnBack.start();
            if (m_isToFar) {
                //for i = 0 to 19 do
