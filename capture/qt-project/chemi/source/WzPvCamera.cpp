...
void WzPvCamera::run()
{
    ...
    if (nullptr == m_cameraCallback) {
        ...
        if (initPVCAM()) {
            if (openCamera()) {
                if (1==1 || m_cameraCallback->cameraSN(QString(m_camSerNum))) {
                    setTemperature(-30);
                    m_isConnected = true;
            ...
            if (!isStartedPreview) {
                ...
                if (getLatestFrame(&frameBytes)) {
...

                    {
                        ...
                        memcpy(m_previewImage16bit, frameBytes,
...
bool WzPvCamera::setupPreview() {
    ...
    size_t speedIndex = 0;
    QVariant isSlow;
    getParam("IsSlowPreview", isSlow);
    if (isSlow.toBool()) {
        for (size_t i = 0; i < m_SpeedTable.size(); i++) {
            if (m_SpeedTable[i].readoutFrequency == 25) {
...
bool WzPvCamera::singleCaptureSetup() {
    ...
    // 用最低速率和最高增益
    // 2021-8-16 补充, 紫外拍摄时信号很强, 用慢速率读取会有拖影现象, 所以在上层业务逻辑中会设置成25MHz来拍摄
    QVariant readoutFrequency;
    getParam("readoutFrequency", readoutFrequency);
    size_t speedIndex = 0;
    if (readoutFrequency.isValid() && readoutFrequency.toFloat() > 0) {
        for (size_t i = 1; i < m_SpeedTable.size(); i++) {
            if (m_SpeedTable[i].readoutFrequency == readoutFrequency.toFloat()) {
                speedIndex = i;
                break;
            }
        }
...
void WzPvCamera::updateTemperature() {
    ...
    if (PV_OK != pl_get_param(m_hCam, PARAM_TEMP, ATTR_CURRENT, reinterpret_cast<void*>(&temperature))) {
        int16 errCode = pl_error_code();
        printErrorMessage(errCode, "pl_get_param(PARAM_TEMP) error");
        if (errCode == PL_ERR_DEVICE_REMOVED) {
            if (m_cameraState != WzCameraState::Disconnected) {
                newCameraState(WzCameraState::Disconnected);
            }
        }
        return;
    ...
    if (m_temperature < 0) {
        m_temperature = m_temperature * 1.3;
    }
    if (m_temperature < -30)
        m_temperature = -30;
}
