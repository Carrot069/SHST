...
void LibusbThread::run() {
    QString action;
    LibusbHandleEventsThread* handleEventsThread = nullptr;
    forever {
        m_mutex.lock();
        ...
        if (isEqual) {
            ...
            if (desc.iSerialNumber) {
                ...
                if (LIBUSB_SUCCESS == ret) {
                    ret = libusb_get_string_descriptor_ascii(handle, desc.iSerialNumber, sn, sizeof(sn));
                    if (ret > 0) {
                        deviceMap["sn"] = QString(reinterpret_cast<char*>(&sn[0]));
                    } else {
                        qDebug() << libusbErrorToJsonStr(ret);
                    }
                    libusb_close(handle);
                }
            }
...
QString LibusbThread::openDevice(const QString& action) {
    ...
    if (iSerialNumber) {
        ...
        int ret = libusb_get_string_descriptor_ascii(handle, iSerialNumber, sn, sizeof(sn));
        if (ret > 0) {
            results["sn"] = QString::fromLocal8Bit(reinterpret_cast<char*>(sn));
        }
    }
