...
import WzUtils 1.0
import "../WzControls"
import "../Controller/PageImageController.js" as PIC
...
Rectangle {
    ...
    property alias listModelSelected: listModelSelected
    property bool keyControlPressed: false
    ...
    Keys.onPressed: {
        console.debug("thumbnail.onPressed")
        if (WzUtils.isMac()) {
    ...
    Keys.onReleased: {
        console.debug("thumbnail.onRelease")
        if (WzUtils.isMac()) {
            if (event.key === Qt.Key_Shift) {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        if (keyControlPressed) {
                            setSelected(model.index, !isSelected(model.index))
                        } else {
                            clearSelected()
                            setSelected(model.index, !isSelected(model.index))
                        }
    ...
    ListModel {
        id: listModelSelected
    }

    // private function
    function isSelected(index) {
        var imageFile = listModel.get(index).imageFile
        for (var i = 0; i < listModelSelected.count; i++) {
            if (listModelSelected.get(i).imageFile === imageFile)
                return true
        }
        return false
    }
    function setSelected(index, selected) {
        var imageFile = listModel.get(index).imageFile
        if (selected) {
            listModelSelected.append(listModel.get(index))
        } else {
            var i = 0
            while (i < listModelSelected.count) {
                if (listModelSelected.get(i).imageFile === imageFile)
                    listModelSelected.remove(i)
                else
                    i++
            }
        }
